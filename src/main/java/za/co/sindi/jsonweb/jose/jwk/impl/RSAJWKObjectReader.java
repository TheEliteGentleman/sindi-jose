...
public class RSAJWKObjectReader extends AbstractJWKObjectReader<RSAJWK, RSAJWKObjectBuilder> implements JWKObjectReader<RSAJWK> {
	...
	protected void build(RSAJWKObjectBuilder jwkObjectBuilder, JSONObject jsonObject) throws Exception {
		// TODO Auto-generated method stub
		PreConditions.checkState(!jsonObject.isNull(JWKConstants.RSA_PARAM_MODULUS), "No JWK RSA parameter '" + JWKConstants.RSA_PARAM_MODULUS + "' found.");
		PreConditions.checkState(!jsonObject.isNull(JWKConstants.RSA_PARAM_EXPONENT_PUBLIC), "No RSA Curve parameter '" + JWKConstants.RSA_PARAM_EXPONENT_PUBLIC + "' found.");
		...
		if (!jsonObject.isNull(JWKConstants.RSA_PARAM_EXPONENT_PRIVATE)) {
			jwkObjectBuilder.setPrivateExponent(new Base64URLUInt(jsonObject.getString(JWKConstants.RSA_PARAM_EXPONENT_PRIVATE)));
			
			PreConditions.checkState(!jsonObject.isNull(JWKConstants.RSA_PARAM_FIRST_PRIME_FACTOR), "JWK RSA key requires the '" + JWKConstants.RSA_PARAM_FIRST_PRIME_FACTOR + "' parameter.");
			jwkObjectBuilder.setFirstPrimeFactor(new Base64URLUInt(jsonObject.getString(JWKConstants.RSA_PARAM_FIRST_PRIME_FACTOR)));
			
			PreConditions.checkState(!jsonObject.isNull(JWKConstants.RSA_PARAM_SECOND_PRIME_FACTOR), "JWK RSA key requires the '" + JWKConstants.RSA_PARAM_SECOND_PRIME_FACTOR + "' parameter.");
			jwkObjectBuilder.setSecondPrimeFactor(new Base64URLUInt(jsonObject.getString(JWKConstants.RSA_PARAM_SECOND_PRIME_FACTOR)));
			
			PreConditions.checkState(!jsonObject.isNull(JWKConstants.RSA_PARAM_FIRST_FACTOR_CRT_EXPONENT), "JWK RSA key requires the '" + JWKConstants.RSA_PARAM_FIRST_FACTOR_CRT_EXPONENT + "' parameter.");
			...
			PreConditions.checkState(!jsonObject.isNull(JWKConstants.RSA_PARAM_SECOND_FACTOR_CRT_EXPONENT), "JWK RSA key requires the '" + JWKConstants.RSA_PARAM_SECOND_FACTOR_CRT_EXPONENT + "' parameter.");
